...
def seirhetuniplus_model(y, t, beta, gamma, alpha, k, a, b, N):

    N = phi(0,y[0],a,b) + y[3] + y[4] + y[5]
    s = -beta * y[0] * y[2]
    U = beta * phi(1, s, a, b) * y[2] - alpha * y[1]
    V = alpha * y[1] - gamma * y[2]
    E = beta * phi(1, s, a, b) * y[2] - alpha * y[3]
    I = alpha * y[3] - gamma * y[4]
    R = gamma * y[4]
    return phi(0, y[0], a, b), U, V, E, I, R

def solve_SEIRhetuniplus(x, beta, gamma, alpha, k, a, b, N, s0, ur = 1):


    R0 = 0
    E0 = I0 = 0.5 * (1 - phi(0, s0, a, b) )
    U0 = E0
    V0 = I0

    results = integrate.odeint(seirhetuniplus_model, (s0, U0, V0, E0, I0, R0), x, args=(beta, gamma, alpha, k, a, b, N))
    ...
    diff = [ur * 0.5 * (1 - phi(0, s0, a, b)) * N]

    for i in range(1, len(results[:, 5])):
        diff.append(N * results[:, 5][i] - N * results[:, 5][i - 1])
