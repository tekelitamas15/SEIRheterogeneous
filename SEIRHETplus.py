...
def seirhetplus_model(y, x, beta, gamma, alpha, p, N):
    ...
    E = beta * (1 + (1 / p)) * pow((y[0]/N), 1 + (2 / p)) * y[2] - alpha * y[1]
    I = alpha * y[1] - gamma * y[2]
    R = gamma * y[2]
    return S, E, I, R


def solve_SEIRhetplus(x, beta, gamma, alpha, p, E0, N, ur = 3.8):
    """
    ...
    results = integrate.odeint(seirhetplus_model, (S0, E0, I0, R0), x, args=(beta, gamma, alpha, p, N))
    diff = [ur * E0]
