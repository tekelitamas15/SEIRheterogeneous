...
def seirhetplus_model(y, x, beta, gamma, alpha, p, N):
    """
    ...
    """
    N = y[0] + y[3] + y[4] + y[5]  # Ensure the total population size remains consistent.
    S = -beta * pow((y[0]/N), 1 + (1 / p)) * y[2]
    U = beta * (1 + (1 / p)) * pow((y[0]/N), 1 + (2 / p)) * y[2] - alpha * y[1]
    V = alpha * y[1] - gamma * y[2]
    E = beta * pow((y[0]/N), 1 + (1 / p)) * y[2] - alpha * y[3]
    I = alpha * y[3] - gamma * y[4]
    R = gamma * y[4]
    return S, U, V, E, I, R


def solve_SEIRhetplus(x, beta, gamma, alpha, p, E0, N, ur = 1):
    """
    ...
    results = integrate.odeint(seirhetplus_model, (S0, U0, V0, E0, I0, R0), x, args=(beta, gamma, alpha, p, N))
    diff = [ur * E0]
    for i in range(1, len(results[:, 5])):
        diff.append(results[:, 5][i] - results[:, 5][i-1])
    return diff

    # diff = [1 - ur * S0]
    # for i in range(1, len(results[:, 0])):
    #     diff.append(results[:, 5][i])
    # return diff

    # diff = [ur * E0 + ur * I0]
    # for i in range(1, len(results[:, 0])):
    #     diff.append(results[:, 3][i] + results[:, 4][i])
    # return diff
